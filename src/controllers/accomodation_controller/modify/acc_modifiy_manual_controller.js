...
// =================================================
// 숙소 메뉴얼 수정 컨트롤러 //
async function acc_modifiy_manual_controller(req, res, next){
    const accomodation_id = req.params.house

    const user = new user_dto({token : req.cookies.auth_token})
    const accomodation = new accomodation_dto({manual : req.body.manual, _id : accomodation_id})
    const accomodation_modfiy_service_layer = new accomodation_modfiy_service()

    const result = await accomodation_modfiy_service_layer.modify_manual(user, accomodation)
    res.status(200).json(result)
}

module.exports = {
    acc_modifiy_manual_controller : async_handler(acc_modifiy_manual_controller)
}