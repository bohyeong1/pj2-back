...
// =================================================
// 숙소 이름 수정 컨트롤러 //
async function acc_modify_title_controller(req, res, next){
    const accomodation_id = req.params.house

    const user = new user_dto({token : req.cookies.auth_token})
    const accomodation = new accomodation_dto({title : req.body.title, _id : accomodation_id})
    const accomodation_modfiy_service_layer = new accomodation_modfiy_service()

    const result = await accomodation_modfiy_service_layer.modify_title(user, accomodation)
    res.status(200).json(result)
}

module.exports = {
    acc_modify_title_controller : async_handler(acc_modify_title_controller)
}