...
async function acc_modify_service_facility_controller(req, res, next){
    ...
    const accomodation = new accomodation_dto({service_facility : req.body.service_facility, _id : accomodation_id})
    const accomodation_modfiy_service_layer = new accomodation_modfiy_service()

    const result = await accomodation_modfiy_service_layer.modify_service_facility(user, accomodation)
    res.status(200).json(result)
}

module.exports = {
    acc_modify_service_facility_controller : async_handler(acc_modify_service_facility_controller)
}