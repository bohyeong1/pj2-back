...
async function acc_modify_photo_controller(req, res, next){
    ...
    const delete_sub_img = check_array(parsing_delete_sub_img) && parsing_delete_sub_img.length ? 
                                       parsing_delete_sub_img : 
                                       null

    const user = new user_dto({token : req.cookies.auth_token})
    const accomodation = new accomodation_dto({main_img : img_files.mainImg ? img_files.mainImg[0].mimetype : null, 
